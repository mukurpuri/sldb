...
class CardHeaderButton extends React.Component {
  ...
  render() {
    ...
    return (
        ...
        <div className="box">
        <div onClick={() => this.props.setTab("headerButton", !minimizer)} className="head bordered">
            HEADER BUTTON PROPERTIES
            <span className="minimizer-icon">
              <span className={`slds-icon_container slds-icon-utility-${this.state.minimizer ? "dash": "add"}`}>
                <svg className="slds-icon slds-icon-text-default slds-icon_xx-small" aria-hidden="true">
                  <use href={`/assets/icons/utility-sprite/svg/symbols.svg#${this.state.minimizer ? "dash": "add"}`}></use>
                </svg>
              </span>
            </span>
        </div>
